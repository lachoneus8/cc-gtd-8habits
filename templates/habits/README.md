...
Home for the teaching, reflection, and practice content behind each of the 7 Habits (and later
the 8th Habit). Each habit gets its own file here once it's introduced, following the same shape
described below so the system feels consistent as you move from one habit to the next.
...
## What Each Habit's File Contains

- **Principle** — the core idea in plain language, not a book summary or a long quote.
- **Reflection Prompts** — a small set of concrete questions to sit with, not a form to fill out.
- **Practice** — one or two concrete things to actually do.
- **Calibration Criteria** — what `/weekly` looks for as evidence of the habit, drawn from your
  own journal and daily entries, assessed 🟢 Aligned / 🟡 Drifting / 🔴 Off, and compared against
  last week so you can see the trend, not just a snapshot.
